...
- 首次启动 API Key 配置窗口，支持安全保存、轻量连接测试、更换和清除 Key。
- 支持只显示中文或英文 + 中文。
- 支持 Low Latency 2 秒、Balanced 3 秒、Stable 4 秒和 Extra Stable 5 秒模式。
- 支持 9 种源语言选项和 9 种目标语言选项。
- 普通设置保存到用户 AppData；API Key 仅从环境变量或 Windows Credential Manager 读取，不写入 `settings.json`。
- 英文 final、翻译和 Overlay 使用 sequence id 跟踪；Stop 最多等待 10 秒完成翻译队列。
- 检测运行中的默认播放设备切换，并提示 Stop 后重新 Start。
- 提供 self-contained Windows x64 安装器，包含开始菜单快捷方式、可选桌面快捷方式和卸载程序。
...
- 已验证 API Key 保存、读取、清除、环境变量优先级，以及无 Key 时阻止 Start 并打开配置提示。
- `Test API Key` 使用单次只读 OpenAI 模型列表请求，不启动字幕主链路。
- 诊断日志默认关闭。
...
- 如果环境变量 `OPENAI_API_KEY` 已设置，它始终优先；应用内 `Clear Key` 不会删除环境变量。
...
- 当前只有浅色主界面主题。
- 安装器尚未代码签名；不包含自动更新、本地 Whisper 或 Realtime API。
